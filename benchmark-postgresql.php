...
/**
 * PHP Script to benchmark PHP and PostgreSQL-Server
...
// Optional: postgres performance test
$options['db.host'] = '';
$options['db.user'] = '';
$options['db.pw'] = '';
$options['db.name'] = '';
...
function test_benchmark($settings)
{
    ...
    if (isset($settings['db.host'])) {
        test_postgresql($result, $settings);
    }
...
function test_postgresql(&$result, $settings)
{
    $timeStart = microtime(true);

    $conn_string = "host=".$settings['db.host']." port=5432 dbname=".$settings['db.name']." user=".$settings['db.user']." password=".$settings['db.pw']."";
    $link = pg_connect($conn_string);
    $result['benchmark']['postgresql_connect'] = timer_diff($timeStart) . ' sec.';

    $dbResult = pg_query($link, 'SELECT VERSION() as version;');
    $arr_row = pg_fetch_array($dbResult);
    $result['sysinfo']['postgresql_version'] = $arr_row['version'];
    $result['benchmark']['postgresql_query_version'] = timer_diff($timeStart) . ' sec.';

    $query = "SELECT BENCHMARK(1000000, decrypt('F3229A0B371ED2D9441B830D21A390C3', 'hello', 'aes'));";
    pg_query($link, $query);
    $result['benchmark']['postgresql_query_benchmark'] = timer_diff($timeStart) . ' sec.';

    pg_close($link);

    $result['benchmark']['postgresql_total'] = timer_diff($timeStart) . ' sec.';
...
function print_benchmark_result($data, $showServerName = true)
{
    ...
    if (isset($data['sysinfo']['postgresql_version'])) {
        $result .= '<thead><tr><th>PostgreSQL</th><th></th></tr></thead>';
        $result .= '<tbody>';
        $result .= '<tr><td>PostgreSQL Version</td><td>' . h($data['sysinfo']['postgresql_version']) . '</td></tr>';
        $result .= '<tr><td>PostgreSQL Connect</td><td>' . h($data['benchmark']['postgresql_connect']) . '</td></tr>';
        $result .= '<tr><td>PostgreSQL Query Version</td><td>' . h($data['benchmark']['postgresql_query_version']) . '</td></tr>';
        $result .= '<tr><td>PostgreSQL Benchmark</td><td>' . h($data['benchmark']['postgresql_query_benchmark']) . '</td></tr>';
        $result .= '<tr class="even"><td>PostgreSQL Total</td><td>' . h($data['benchmark']['postgresql_total']) . '</td></tr>';
        $result .= '</tbody>';
    }
